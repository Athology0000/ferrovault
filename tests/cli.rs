...
fn init_add_get_list_delete() {
    ...
    assert_eq!(
        commands::cmd_list(&store, b"m").unwrap(),
        vec!["github".to_string()]
    );

    commands::cmd_delete(&store, b"m", "github").unwrap();
    assert!(matches!(
        commands::cmd_get(&store, b"m", "github"),
...
fn add_duplicate_fails() {
    ...
    assert!(matches!(
        commands::cmd_add(&store, b"m", "x", entry("b")).unwrap_err(),
        Error::EntryExists(_)
    ));
}
...
fn delete_missing_fails() {
    ...
    commands::cmd_init(&store, b"m").unwrap();
    assert!(matches!(
        commands::cmd_delete(&store, b"m", "nope").unwrap_err(),
